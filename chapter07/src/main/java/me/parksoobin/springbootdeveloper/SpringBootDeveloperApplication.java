...
/*
    1. 사전 지식 : 타임리프(Thymeleaf)
        타임리프는 템플릿 엔진입니다. 다만 템플릿 엔진은 HTML 과 템플릿 엔진을 위한 문법을 섞어서 사용
            ...
            -> 저희는 다음 과정에서 HTML 을 배우게 되므로 아주 간단한 화면 구성을 위해서 코드를 작성 할 예정


        * 템플릿 엔진 : 스프링 서버에서 데이터를 받아 우리가 보는 웹 페이지, 즉 HTML 상에서 그 데이터를 넣어 보여주는 도구
        ...
        템플릿 엔진 개념 잡기
            ...
            p 태그도 비슷한 상황입니다.
            이상이 템플릿 문법의 예시인데,
            이렇게 해두면 서버에서 이름, 나이라는 key 로 데이터를 템플릿 엔진에 넘겨주고 템플릿 엔진은 이를 받아 HTML 에 값을 적용합니다.
            ...
            템플릿 엔진은 각각 문법이 조금씩 다른 편입니다. 그래서 템플릿 엔진마다 문법을 새로 배워야 하는 단점은 존재합니다만
            구조는 그래도 비슷합니다.
                대표적인 템플릿 엔진 예시 : JSP, 타임리프, 프리마커 등

                하지만 스피링 타임리프를 권장하고 있으므로 우리는 타임리프 사용 예정

            타임리프 표현식과 문법
                타임리프의 문법은 직관적인 편인데, 자주 사용하는 표현식과 문법은 예시로만 보여드리고,
                구체적인 사용 방법은 실습으로 알아볼겁니다.

                이하에서 소개하는 표현식들은 전달받은 데이터를 사용자들이 볼 수 있게 뷰로 만들기 위해 사용되는 표현식 예시입니다.
