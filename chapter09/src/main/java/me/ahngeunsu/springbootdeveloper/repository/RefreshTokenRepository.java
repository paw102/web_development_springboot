...
/*
    토큰 필터 구현
        필터는 각종 요청을 실제로 처리하기 위한 로직
            -> 전달 전 후로 URL 패턴에 맞는 모든 요청을 처리하는 기능을 제공
        요청이 오면 헤더값을 비교해서 토큰이 있는 지를 확인하고 유효 토큰이라면
        ...
        시큐리티 컨텍스트 (Security Context) : 인증 객체가 저장되는 보관소
            ...
            코드 아무 곳에서나 참조 할 수 있고, 다른 스레드와 공유하지 않으므로 독립적으로 사용 가능.

            시큐리티 컨텍스트 객체가 저장하는 객체 시큐리티 컨텍스트 홀더에 해당함.

            config -> TokenAuthenticationFilter.java 파일 생성
            이 필터는 -> 엑세스 토큰이 담긴 Authentication 헤더 값을 가져온 뒤에
            엑세스 토큰이 유효하다면 인증 정보를 설정
 */